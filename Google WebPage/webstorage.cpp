...
//unioning together all of the webpages that have at least one word in common with the words
//we are querrying
set<string> WebStorage::union_webpages(set<string> partial_webpages, set<string> resulting_webpages)
{
	//getting an iterator to point at the begining of the 
	set<string>::iterator it;
	it = partial_webpages.begin();
	//iterating over all of the new webpages we got from searching for a word in the map
	//and returning a set
...
//intersect webpages which will only allow for our returning webpage to contain all of the webpages that
//contains all of the words that we want to see in the webpage
set<string> WebStorage::intersect_webpages(set<string> partial_webpages, set<string> resulting_webpages)
{
	...
	set<string> temp_webpage_set;
	//if we haven't inserted anything into the resulting webpage set yet have it be equal
	...
	else
	{
		...
		{
			//if the word in partial webpages we are on is the same as the word we are on for 
			//resulting webpages
			if(*it == *it2)
			{
				//insert the webpage into the temp webpage that has the new intersection
